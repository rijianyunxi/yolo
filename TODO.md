...
- [x] 上传标签文件在导入时校验：除扩展名 `.txt` 外，校验格式、坐标范围、类别 ID（导入走 `validate_yolo_label_file`，标注保存走 Pydantic 边界校验）
...
- [x] 增加训练/数据出错的可视化提示，避免静默失败（任务失败时顶部横幅提示，可一键查看日志）
...
- [x] 前端所有页面均为中文文案
- [x] 能用 UI 完成：导入图片与标签→在线标注→数据检查→训练（试训/正式）→查看日志→预测调试（对应 API 全部就绪并通过冒烟验证）
- [x] 切换 profile（cat / safety 等）后，标注类别、训练数据、best.pt 都能自动跟随
- [x] 新增一个 profile 时，理论上只需加一个 YAML 和目录，不动代码（`webui/config.py` 自动扫描 `datasets/*/*.yaml`）
